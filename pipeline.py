...
if program_selection == '1':
    ...
    if train_dataframe.columns[0] == 'ts':  # zeek
        ...
        train_dataframe.drop(columns=['uid', 'tunnel_parents'], inplace=True)
        test_dataframe.drop(columns=['uid', 'tunnel_parents'], inplace=True)

        categorical_features = ['id.orig_h', 'id.resp_h', 'proto', 'service',
                                'conn_state', 'local_orig', 'local_resp', 'history']

        numerical_features = ['ts', 'id.orig_p', 'id.resp_p', 'duration', 'orig_bytes', 'resp_bytes', 'missed_bytes',
        ...
        train_dataframe['StartTime'] = pd.to_datetime(train_dataframe['StartTime'],
                                                      format='%H:%M:%S.%f', errors='coerce')
        test_dataframe['StartTime'] = pd.to_datetime(test_dataframe['StartTime'],
                                                     format='%H:%M:%S.%f', errors='coerce')

        train_dataframe = train_dataframe.dropna(subset=['StartTime'])
        test_dataframe = test_dataframe.dropna(subset=['StartTime'])

        train_dataframe['StartTime'] = train_dataframe['StartTime'].apply(lambda x: x.timestamp())
        test_dataframe['StartTime'] = test_dataframe['StartTime'].apply(lambda x: x.timestamp())
        ...
        categorical_features = ['Flgs', 'Proto', 'SrcAddr', 'Sport', 'Dir', 'DstAddr', 'Dport', 'State']
        ...
        categorical_features = ['%dir', 'flowStat', 'numHdrDesc', 'hdrDesc', 'srcMac', 'dstMac', 'ethType',
                                'srcIP', 'srcIPOrg', 'dstIP', 'dstIPOrg', 'macStat', 'srcMac_dstMac_numP',
                                'srcMacLbl_dstMacLbl', 'dstPortClass', 'tcpFStat', 'ipToS', 'ipFlags', 'ipOptCpCl_Num',
                                'ip6OptCntHH_D', 'ip6OptHH_D', 'tcpFlags', 'tcpAnomaly', 'tcpOptions', 'tcpMPTBF',
                                'tcpMPF', 'tcpMPDSSF', 'tcpStatesAFlags', 'icmpStat', 'icmpBFTypH_TypL_Code',
                                'icmpTmGtw', 'pktAsm', 'bytAsm']

        numerical_features = ['flowInd', 'timeFirst', 'timeLast', 'duration', 'srcPort',
                              'dstPort', 'dstPortClassN', 'vlanID', 'l4Proto', 'macPairs', 'tcpMPAID',
                              'numPktsSnt', 'numPktsRcvd', 'numBytesSnt', 'numBytesRcvd', 'minPktSz', 'maxPktSz',
                              'avePktSize', 'stdPktSize', 'minIAT', 'maxIAT', 'aveIAT', 'stdIAT', 'pktps', 'bytps',
                              'ipMindIPID', 'ipMaxdIPID', 'ipMinTTL', 'ipMaxTTL', 'ipTTLChg',
                              'ipOptCnt', 'tcpISeqN', 'tcpPSeqCnt', 'tcpSeqSntBytes', 'tcpSeqFaultCnt', 'tcpPAckCnt',
                              'tcpFlwLssAckRcvdBytes', 'tcpAckFaultCnt', 'tcpBFlgtMx', 'tcpInitWinSz', 'tcpAveWinSz',
                              'tcpMinWinSz', 'tcpMaxWinSz', 'tcpWinSzDwnCnt', 'tcpWinSzUpCnt', 'tcpWinSzChgDirCnt',
                              'tcpWinSzThRt', 'tcpOptPktCnt', 'tcpOptCnt', 'tcpMSS', 'tcpWS', 'tcpTmS', 'tcpTmER',
                              'tcpEcI', 'tcpUtm', 'tcpBtm', 'tcpSSASAATrip', 'tcpRTTAckTripMin', 'tcpRTTAckTripMax',
                              'tcpRTTAckTripAve', 'tcpRTTAckTripJitAve', 'tcpRTTSseqAA', 'tcpRTTAckJitAve', 'icmpTCcnt',
                              'icmpEchoSuccRatio', 'icmpPFindex', 'connSip', 'connDip', 'connSipDip', 'connSipDprt',
                              'connF', 'connG', 'connNumPCnt', 'connNumBCnt']

        train_dataframe.drop(columns=['numHdrs', 'srcIPCC', 'dstIPCC'], inplace=True)
        test_dataframe.drop(columns=['numHdrs', 'srcIPCC', 'dstIPCC'], inplace=True)
    ...
    if classifier_selection == '1':  # Naive Bayes
        ...
        model = Pipeline(steps=[
            ('preprocessor', preprocessor),
            ('classifier', RandomForestClassifier(verbose=10))
        ])
